...
var expectedMetricNames = map[string]int{
	...
	"gcs_read_count":              1,
	"gcs_read_bytes_count":        0, // TODO: cannot find this metric in e2e tests.
	"gcs_reader_count":            2,
	"gcs_request_count":           6,
...
func (t *gcsFuseCSIMetricsTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	ginkgo.It("should emit metrics", func() {
		init(specs.EnableFileCacheWithMetricsEnabledPrefix)
		defer cleanup()
		...
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName, fmt.Sprintf("wget -O %v/metrics.prom http://%v:8080/metrics", mountPath, csiPodIP))
